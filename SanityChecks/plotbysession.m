...
load fakedatasets_VEO
sessions = datasets;

load modelfitting_fakedataVEO
...
% %criterion diffs from VSO model
% AIC_diff = [AIC_all(:,2)-AIC_all(:,1) AIC_all(:,3)-AIC_all(:,1)];
% AICc_diff = [AICc_all(:,2)-AICc_all(:,1) AICc_all(:,3)-AICc_all(:,1)];
% BIC_diff = [BIC_all(:,2)-BIC_all(:,1) BIC_all(:,3)-BIC_all(:,1)];
% BMC_diff = [BMC_all(:,2)-BMC_all(:,1) BMC_all(:,3)-BMC_all(:,1)];
...
datacond_ste = nanstd(perfdata1,0,1)/sqrt(length(sessions));
datadelta_ste = squeeze(nanstd(perfdata2,0,1))/sqrt(length(sessions));
VSO1_ste = std(VSO1_all,0,1)/sqrt(length(sessions));
VSO2_ste = squeeze(std(VSO2_all,0,1))/sqrt(length(sessions));
VEO1_ste = std(VEO1_all,0,1)/sqrt(length(sessions));
VEO2_ste = squeeze(std(VEO2_all,0,1))/sqrt(length(sessions));
VVO1_ste = std(VVO1_all,0,1)/sqrt(length(sessions));
VVO2_ste = squeeze(std(VVO2_all,0,1))/sqrt(length(sessions));
pars_VSO_ste = std(pars_VSO_all,0,1)/sqrt(length(sessions));
pars_VEO_ste = std(pars_VEO_all,0,1)/sqrt(length(sessions));
pars_VVO_ste = std(pars_VVO_all,0,1)/sqrt(length(sessions));
...
% Rs_VSO_ste = std(Rs_VSO_all,0,1)/sqrt(length(sessions));
% Rs_VEO_ste = std(Rs_VEO_all,0,1)/sqrt(length(sessions));
% Rs_VVO_ste = std(Rs_VVO_all,0,1)/sqrt(length(sessions));
...
set(gca,'XTick',10:10:90)
errorbar(repmat(deltavec,4,1)', datadelta_mean', datadelta_ste','o'); hold on;
xlabel('Change magnitude'); ylabel('Proportion correct');axis([7 93 0.4 1.00]);
legend(strcat('Condition = ',int2str(condvec')), 4, 'Location', 'SouthEast');
title('VSO model fit to VVOnew fake dataset')
...
legend(strcat('Condition = ',int2str(condvec')), 4, 'Location', 'SouthEast');
title('VEO model fit to VVOnew fake dataset')
...
xlabel('Change magnitude'); ylabel('Proportion correct');axis([7 93 0.4 1.0]);
legend(strcat('Condition = ',int2str(condvec')), 4, 'Location', 'SouthEast');
title('VVO model fit to VVOnew fake dataset')
...
ylabel('Model log likelihood relative to VVO new');
title('Model comparison of fits from VVOnew fake dataset')
